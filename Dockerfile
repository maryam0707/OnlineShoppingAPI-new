#See https://aka.ms/containerfastmode to understand how Visual Studio uses this Dockerfile to build your images for faster debugging.

#Depending on the operating system of the host machines(s) that will build or run the containers, the image specified in the FROM statement may need to be changed.
#For more information, please see https://aka.ms/containercompat

FROM mcr.microsoft.com/dotnet/aspnet:6.0 AS base
WORKDIR /app
EXPOSE 80
EXPOSE 443
EXPOSE 9092 

FROM mcr.microsoft.com/dotnet/sdk:6.0 AS build
WORKDIR /src
COPY ["OnlineShoppingAPI/OnlineShoppingAPI.csproj", "OnlineShoppingAPI/"]
RUN dotnet restore "OnlineShoppingAPI/OnlineShoppingAPI.csproj"
COPY . .
WORKDIR "/src/OnlineShoppingAPI"
RUN dotnet build "OnlineShoppingAPI.csproj" -c Release -o /app/build

FROM confluentinc/cp-kafka:latest AS kafka

FROM build AS publish
RUN dotnet publish "OnlineShoppingAPI.csproj" -c Release -o /app/publish /p:UseAppHost=false

FROM base AS final
WORKDIR /app
COPY --from=publish /app/publish .
COPY --from=kafka /etc/kafka /etc/kafka
CMD ["bash", "-c" , "zookeeper-server-start /etc/kafka/zookeeper.properties & kafka-server-start /etc/kafka/server.properties"]
ENTRYPOINT ["dotnet", "OnlineShoppingAPI.dll"]